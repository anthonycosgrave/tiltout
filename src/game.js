...
Game.prototype.updateBonus = function()
{
	if (this.bonus.active)
	{
		if ( (this.bonus.x <= myGlobal.touchX && myGlobal.touchX <= (this.bonus.x + this.bonus.width))
			&& (this.bonus.y <= myGlobal.touchY && myGlobal.touchY <= (this.bonus.y + this.bonus.height)))
		{
		...
		if (!this.isUpdatingBonusColour)
		{
			...
			setTimeout(function() {strobeBonusColour(that)}, 100);
		}
	}
...
Game.prototype.checkForCollisions = function()
{
	try
	{
		for (var i = this.balls.length-1; i >= 0 ; i--)
		{
			...
			if (row - 2 >= 0)
			{
				if (currentBall.y < height && row >= 0
					&& col >= 0 && this.bricks[row - 2][col] == 1)
				{
			...
			else if (currentBall.y + currentBall.radius > this.player.y
				&& currentBall.x >= this.player.x
				&& currentBall.x <= (this.player.x + this.player.width))
			{
